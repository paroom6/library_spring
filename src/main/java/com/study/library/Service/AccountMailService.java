...
public class AccountMailService {
    ...
    public boolean sendAuthMail() {
        ...
        try {
            ...
            mailContent.append("<h1>계정활성화</h1>");
    ...
    public Map<String, Object> authenticate(String token) {
        ...
        try {
            ...
            RoleRegister roleRegister = userMapper.findRoleRegisterByRoleIdAndRoleId(Integer.parseInt(claims.get("userId").toString()), 2);
            if(roleRegister != null) {
                return Map.of("status", true    , "message", "이미 인증이 완료된 계정입니다.");
            } else {
                userMapper.saveRole(userId, 2);
                resultMap = Map.of("status", true, "message", "인증 완료.");
            }
            //IllegalAccessException ==> null 또는 빈값
